        ...
        .map(|table| {
            ...
            let expressions = preferred_order
                ...
                .map(|column_name| {
                    ...
                    let expression = if column_types.contains_key(column_name.as_str()) {
...
#[cfg(test)]
mod tests {
    use super::{SemanticNamingStrategy, semantic_view_base_name};

    #[test]
    fn strips_year_tokens_from_semantic_view_names() {
        let name = semantic_view_base_name(
            "participant_registration_2026",
            "dudetail_2025",
            SemanticNamingStrategy::StripYearTokens,
        );
        assert_eq!(name, "participant_registration_dudetail");
    }

    #[test]
    fn keeps_default_semantic_view_names_unchanged() {
        let name = semantic_view_base_name(
            "participant_registration_2026",
            "dudetail_2025",
            SemanticNamingStrategy::Default,
        );
        assert_eq!(name, "participant_registration_2026_dudetail_2025");
    }
}
